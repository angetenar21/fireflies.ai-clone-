...
def list_meetings(
    ...
    scope: str = Query("hosted", pattern="^(hosted|shared|all)$"),
    sort: str = Query("recency", pattern="^(recency|oldest)$"),
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
) -> MeetingListResponse:
    base_query = db.query(Meeting).options(*meeting_list_options())
    
    
    if scope == "hosted":
        query = base_query.filter(Meeting.owner_id == user.id)
    elif scope == "shared":
        query = base_query.filter(or_(Meeting.owner_id != user.id, Meeting.owner_id.is_(None)))
    else:
        query = base_query
...
@router.patch("/{meeting_id}/star", response_model=MeetingDetail)
def toggle_star_meeting(
    meeting_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
) -> MeetingDetail:
    """Toggle the starred/bookmarked state of a meeting."""
    meeting = _owned_meeting_or_404(db, meeting_id, user)
