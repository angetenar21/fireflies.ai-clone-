...
export default function DigestPage() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-7.5rem)] bg-white pb-28 dark:bg-aura-bg">
      ...
      <div className="border-b border-[#E5E7EB] px-5 py-3 dark:border-aura-border sm:px-8">
        <div className="flex flex-wrap items-center gap-1.5 text-[12px] text-aura-gray">
          ...
          <span>/</span>
          <span className="font-medium text-aura-text">Daily Digest</span>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[820px] px-5 py-6 sm:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-[26px] font-semibold tracking-tight text-aura-text">
                Daily Digest
              </h1>
              <span className="text-[11px] font-semibold uppercase tracking-wide text-aura-gray-2">
                Productivity
              </span>
            </div>
          ...
          <button
            type="button"
            className="inline-flex items-center gap-2 self-start rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-[13px] font-medium text-aura-text dark:border-aura-border dark:bg-aura-bg"
          >
            <Calendar className="h-3.5 w-3.5 text-aura-gray" />
            This Month
          </button>
        </div>

        {/* Meeting entry bar */}
        <div className="mt-6 flex items-center gap-2 rounded-lg bg-[#F3F4F6] px-3 py-2.5 text-[13px] text-aura-gray dark:bg-[var(--aura-input-bg)]">
          <Video className="h-3.5 w-3.5 shrink-0" />
          {loading ? (
            <span className="aura-skeleton h-4 w-48" />
          ) : (
            <span>
              1 meeting{" "}
              {meeting
          ...
          <button
            type="button"
            className="ml-auto rounded p-1 text-aura-gray-2 hover:bg-white"
            aria-label="More"
          >
        ...
        <article className="mt-5 space-y-6">
          <section>
            <h2 className="mb-2 flex items-center gap-2 text-[15px] font-semibold text-aura-text">
              <span aria-hidden>📌</span> Daily Digest
            </h2>
            <p className="text-[14px] leading-relaxed text-aura-gray">
          ...
          <section>
            ...
            <ul className="space-y-1.5 text-[14px] text-aura-gray">
              {openTasks.length === 0 ? (
                <li>
                ...
                openTasks.slice(0, 5).map((t) => (
                  <li key={t.id}>
                    • {t.text}
                    {t.assignee ? (
                      <span className="text-aura-gray-2"> — {t.assignee}</span>
                    ) : null}
                  </li>
                ))
              )}
          ...
          {/* Feedback */}
          <div className="border-t border-[#E5E7EB] pt-5 dark:border-aura-border">
            <div className="flex flex-wrap items-center gap-3">
              <Smile className="h-4 w-4 text-aura-gray" />
      ...
      <div className="pointer-events-none fixed bottom-6 left-1/2 z-20 w-[min(560px,calc(100%-2rem))] -translate-x-1/2">
        <div className="pointer-events-auto flex items-center gap-2 rounded-full border border-[#E5E7EB] bg-white px-3 py-2 shadow-lg dark:border-aura-border dark:bg-aura-bg">
          ...
          <input
            ...
            className="flex-1 border-0 bg-transparent text-[13px] text-aura-text outline-none placeholder:text-aura-gray-2"
          />
          <button
            type="button"
            disabled
