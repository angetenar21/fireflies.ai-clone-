...
export default function IntegrationsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1100px] space-y-5">
      <div>
        <h1 className="text-[22px] font-semibold text-aura-text">Integrations</h1>
      </div>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-[13px] font-medium transition ${
                tab === t.key
                  ...
                  : "border-[#E5E7EB] bg-white text-[#4B5563] hover:border-[#D1D5DB] dark:border-aura-border dark:bg-aura-bg dark:text-aura-gray"
              }`}
            >
              {t.label}
              {t.key === "more" ? (
        ...
        <label className="relative block w-full lg:max-w-[220px]">
          ...
          <input
            ...
            placeholder="Search"
            className="w-full rounded-lg border border-[#E5E7EB] bg-white py-2 pl-9 pr-3 text-[13px] outline-none focus:border-[#6C5CE7]/40 dark:border-aura-border dark:bg-aura-bg"
          />
        </label>
      </div>
      ...
      <button
        type="button"
        className="inline-flex items-center gap-1.5 text-[12px] font-medium text-aura-gray hover:text-[#6C5CE7]"
      >
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-[#E5E7EB] px-6 py-12 text-center dark:border-aura-border">
          <p className="text-sm font-semibold text-aura-text">No matches</p>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {filtered.map((item) => (
            <article
              key={item.id}
              className="relative flex flex-col rounded-xl border border-[#E5E7EB] bg-white p-5 shadow-sm transition hover:shadow-md dark:border-aura-border dark:bg-aura-bg"
            >
              ...
              <h2 className="mt-4 text-[15px] font-semibold text-aura-text">
                {item.name}
              </h2>
              <p className="mt-0.5 text-[12px] text-aura-gray-2">Fireflies</p>
              <p className="mt-2 flex-1 text-[13px] leading-relaxed text-aura-gray">
                {item.description}
              </p>
              <button
                type="button"
                ...
                className={`mt-5 w-full cursor-not-allowed rounded-lg border px-3 py-2.5 text-[13px] font-medium ${
                  item.connected
                    ...
                    : "border-[#E5E7EB] bg-[#F9FAFB] text-aura-gray-2 dark:border-aura-border"
                }`}
              >
