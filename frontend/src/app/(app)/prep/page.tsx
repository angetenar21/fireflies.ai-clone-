...
export default function PrepPage() {
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-7.5rem)] bg-white dark:bg-aura-bg">
      {/* Meeting Prep list panel */}
      <aside className="hidden w-[280px] shrink-0 flex-col border-r border-[#E5E7EB] dark:border-aura-border lg:flex">
        <div className="border-b border-[#E5E7EB] p-4 dark:border-[var(--aura-border-soft)]">
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#FCE7F3] text-[#DB2777]">
              <CalendarDays className="h-4 w-4" />
            </span>
            <h2 className="flex-1 text-[15px] font-semibold text-aura-text">
              Meeting Prep
            </h2>
            <button
              type="button"
          ...
          <p className="mt-2 text-[12px] leading-relaxed text-aura-gray">
        ...
        <div className="flex items-center justify-between px-4 py-3">
          <p className="text-[13px] font-semibold text-aura-text">
            {meetings.length} Meeting Prep{meetings.length === 1 ? "" : "s"}
          </p>
          <button
            type="button"
            className="rounded-md p-1 text-aura-gray hover:bg-[#F3F4F6]"
            aria-label="Add"
          >
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4">
          {loading ? (
            <div className="space-y-2 p-2">
              <div className="aura-skeleton h-16 w-full" />
              <div className="aura-skeleton h-16 w-full" />
            </div>
          ) : (
            groups.map((g) => (
              <div key={g.label} className="mb-3">
                <p className="mb-1.5 px-2 text-[11px] font-semibold text-aura-gray-2">
                  {g.label}
                </p>
                        ...
                        >
                          <div className="flex items-start gap-2">
                            <div className="min-w-0 flex-1">
                              <p className="text-[12px] text-aura-gray">
                                {ts}
                              </p>
                              <p
                                className={`truncate text-[13px] font-semibold ${
                                  active ? "text-[#6C5CE7]" : "text-aura-text"
                                }`}
                              >
                                {m.title}
                              </p>
                            </div>
                            <span className="rounded border border-[#E5E7EB] px-1 text-[9px] font-semibold text-aura-gray-2">
                              EN
                            </span>
                          </div>
                        </button>
      ...
      <div className="relative min-w-0 flex-1 overflow-y-auto pb-28">
        <div className="border-b border-[#E5E7EB] px-5 py-3 dark:border-aura-border sm:px-8">
          <div className="flex flex-wrap items-center gap-1.5 text-[12px] text-aura-gray">
            ...
            <span>/</span>
            <span className="font-medium text-aura-text">Meeting Prep</span>
          </div>
        </div>

        <div className="mx-auto max-w-[720px] px-5 py-6 sm:px-8">
          {!selected && !loading ? (
            <p className="py-20 text-center text-sm text-aura-gray">
            ...
            <>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <h1 className="text-[24px] font-semibold tracking-tight text-aura-text">
                    {selected?.title ?? "…"}
                  </h1>
                  <div className="mt-2 flex flex-wrap items-center gap-3 text-[12px] text-aura-gray">
                ...
                <div className="flex shrink-0 flex-col items-end gap-2">
                  ...
                  {selected ? (
                    <Link
                      ...
                      className="inline-flex items-center gap-2 rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-[13px] font-semibold text-aura-text shadow-sm transition hover:border-[#6C5CE7]/40 dark:border-aura-border dark:bg-aura-bg"
                    >
              ...
              <section className="mt-8">
                ...
                <p className="mt-0.5 text-[12px] text-aura-gray-2">
                  Previous sync notes
                </p>
                  ...
                  {topics.slice(0, 6).map((topic, i) => {
                    ...
                    return (
                      <li key={topic} className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-[14px] font-semibold text-aura-text">
                            {topic.length > 48
                              ? topic.slice(0, 48) + "…"
                              : topic}
                          </p>
                          <p className="mt-0.5 text-[13px] text-aura-gray">
              ...
              <section className="mt-8">
                <div className="mb-3 flex items-center gap-2">
                  <CheckSquare className="h-4 w-4 text-aura-gray" />
                  <h2 className="text-[14px] font-semibold text-aura-text">
                    {tasks.length || 0} Tasks
                  </h2>
                </div>
                {tasks.length === 0 ? (
                  <p className="text-[13px] text-aura-gray">
                    ...
                    {tasks.map((item) => (
                      ...
                      >
                        <input
                          ...
                          readOnly
                          className="h-4 w-4 rounded border-aura-border text-[#6C5CE7]"
                        />
                        <span
                          className={`min-w-0 flex-1 text-[14px] ${
                            item.is_completed
                              ? "text-aura-gray line-through"
                              : "text-aura-text"
                          }`}
                        >
                          {item.text}
                        </span>
                        <span className="shrink-0 text-[12px] text-aura-gray">
                          {item.assignee ?? "You"}
                        </span>
                        <button
                          type="button"
                          className="rounded p-1 text-aura-gray-2 hover:bg-[#F3F4F6]"
                          aria-label="More"
                        >
      ...
      <div className="pointer-events-none fixed bottom-6 left-1/2 z-20 w-[min(560px,calc(100%-2rem))] -translate-x-1/2">
        <div className="pointer-events-auto flex items-center gap-2 rounded-full border border-[#E5E7EB] bg-white px-3 py-2 shadow-lg dark:border-aura-border dark:bg-aura-bg">
          ...
          <input
            ...
            className="flex-1 border-0 bg-transparent text-[13px] outline-none placeholder:text-aura-gray-2"
          />
          <button
            type="button"
            disabled
