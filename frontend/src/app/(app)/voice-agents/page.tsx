...
export default function VoiceAgentsPage() {
  ...
  return (
    <div className="relative mx-auto max-w-[1120px] space-y-6 pb-24">
      {/* Tabs */}
      <div className="flex gap-6 border-b border-[#E5E7EB] dark:border-aura-border">
        <button
          type="button"
          ...
          className={`relative pb-3 text-[14px] font-semibold transition ${
            tab === "discover"
              ? "text-aura-text"
              : "text-aura-gray hover:text-aura-text"
          }`}
        >
          Discover
          {tab === "discover" ? (
        ...
        <button
          ...
          onClick={() => setTab("mine")}
          className={`relative pb-3 text-[14px] font-semibold transition ${
            tab === "mine" ? "text-aura-text" : "text-aura-gray hover:text-aura-text"
          }`}
        >
          My Voice Agents (0)
          {tab === "mine" ? (
      ...
      {tab === "mine" ? (
        <div className="rounded-xl border border-[#E5E7EB] bg-white px-6 py-16 text-center dark:border-aura-border dark:bg-aura-bg">
          ...
          <h2 className="mt-4 text-lg font-semibold text-aura-text">
            No voice agents yet
          </h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-aura-gray">
        ...
        <>
            ...
            <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-lg">
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="text-[24px] font-semibold tracking-tight text-aura-text sm:text-[28px]">
                ...
                <p className="mt-2 text-[14px] leading-relaxed text-aura-gray">
                ...
                <div className="mt-5 flex flex-wrap gap-2.5">
                  ...
                  <button
                    type="button"
                    disabled
                    className="inline-flex cursor-not-allowed items-center gap-2 rounded-lg border border-[#6C5CE7]/40 bg-white px-4 py-2.5 text-[13px] font-semibold text-[#6C5CE7] dark:bg-aura-bg"
                  >
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-[18px] font-semibold text-aura-text">
                {firstName}, set up your Voice Agent in 2 minutes
              </h2>
              <button
                type="button"
                className="mt-1 inline-flex items-center gap-1.5 text-[12px] font-medium text-aura-gray hover:text-[#6C5CE7]"
              >
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {agents.map((agent) => {
              ...
              return (
                <article
                  key={agent.title}
                  className="flex flex-col rounded-xl border border-[#E5E7EB] bg-white p-5 shadow-sm transition hover:shadow-md dark:border-aura-border dark:bg-aura-bg"
                >
                  <span
                  ...
                  <h3 className="mt-4 text-[15px] font-semibold text-aura-text">
                    {agent.title}
                  </h3>
                  <p className="mt-1.5 flex-1 text-[13px] leading-relaxed text-aura-gray">
                    {agent.description}
                  </p>
                  <button
                    type="button"
                    disabled
                    className="mt-5 w-fit cursor-not-allowed rounded-lg border border-[#E5E7EB] bg-white px-3.5 py-1.5 text-[13px] font-semibold text-aura-text dark:border-aura-border dark:bg-aura-bg"
                  >
                    Set-Up
                  </button>
                </article>
