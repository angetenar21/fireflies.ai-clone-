  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-start justify-center bg-black/40 px-4 pt-[12vh] backdrop-blur-[2px]">
      <div
        ...
        aria-hidden
      />
      <div className="relative z-10 flex max-h-[70vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-aura-border bg-aura-bg shadow-2xl">
        <div className="flex items-center gap-2 border-b border-aura-border px-4 py-3">
          <Search className="h-4 w-4 text-aura-gray-2" />
          <input
            autoFocus
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search meetings, transcripts, people, actions…"
            className="min-w-0 flex-1 bg-transparent text-sm text-aura-text outline-none placeholder:text-aura-gray-2"
          />
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-aura-gray-2 hover:bg-aura-soft hover:text-aura-purple"
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {loading ? (
            <div className="flex items-center justify-center gap-2 py-12 text-sm text-aura-gray">
          ...
          {!loading && !error && debounced && results.length === 0 ? (
            <p className="px-3 py-8 text-center text-sm text-aura-gray">
          ...
          {grouped.map((group) => {
            ...
            return (
              <div key={group.key} className="mb-3">
                <p className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-aura-gray-2">
                  ...
                  {group.items.slice(0, 8).map((r, i) => (
                    <li key={`${r.match_type}-${r.meeting_id}-${r.line_id ?? i}`}>
                      <button
                        type="button"
                        onClick={() => go(r)}
                        className="flex w-full flex-col gap-0.5 rounded-xl px-3 py-2.5 text-left transition hover:bg-aura-soft"
                      >
                        <span className="truncate text-sm font-semibold text-aura-text">
                          {r.meeting_title}
                        </span>
                        <span className="line-clamp-2 text-xs text-aura-gray">
                          {r.snippet}
                        </span>
                      </button>
                    </li>
