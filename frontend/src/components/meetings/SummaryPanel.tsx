    ...
    <div className="space-y-6">
      <section>
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-aura-text">Overview</h3>
        </div>
        {summary?.overview_text ? (
          <p className="text-sm leading-relaxed text-aura-gray">
            {summary.overview_text}
          </p>
        ) : (
          <p className="rounded-lg border border-dashed border-aura-border bg-[var(--aura-input-bg)] px-3 py-4 text-sm text-aura-gray">
      ...
      <section>
        ...
        {topics.length === 0 ? (
          <p className="text-sm text-aura-gray">No topics extracted yet.</p>
        ) : (
          <ol className="space-y-2">
            {topics.map((topic, i) => (
              <li
                key={topic.id}
                className="flex gap-3 rounded-lg border border-[var(--aura-border-soft)] bg-[var(--aura-input-bg)]/80 px-3 py-2.5"
              >
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-[#F3F0FF] text-xs font-semibold text-[#6C5CE7]">
                  {i + 1}
                </span>
                <span className="text-sm text-aura-text">{topic.topic_text}</span>
              </li>
            ))}
          </ol>
        )}
