...
export function MeetingTagsEditor({ meeting, onSaved }: MeetingTagsEditorProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <Tag className="h-3.5 w-3.5 text-aura-purple" />
      {tags.map((t) => (
        <span
          key={t.id}
          className="inline-flex items-center gap-1 rounded-full bg-aura-soft px-2.5 py-1 text-[11px] font-semibold text-aura-purple"
        >
          {t.name}
          <button
            type="button"
      ...
      <form onSubmit={(e) => void add(e)} className="inline-flex items-center gap-1">
        <input
          ...
          placeholder="Add tag…"
          disabled={busy}
          className="w-28 rounded-full border border-dashed border-aura-border bg-transparent px-2.5 py-1 text-[11px] text-aura-text outline-none placeholder:text-aura-gray-2 focus:border-aura-purple"
        />
        {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin text-aura-purple" /> : null}
      </form>
    </div>
  );
}
