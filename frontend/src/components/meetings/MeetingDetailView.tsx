...
export function MeetingDetailView({ meetingId }: { meetingId: number }) {
  ...
  if (loading) {
    return (
      <div className="flex h-[100vh] flex-col bg-white">
        <div className="flex h-12 items-center gap-3 border-b border-[var(--aura-border-soft)] px-3">
          <div className="aura-skeleton h-5 w-5 rounded" />
          <div className="aura-skeleton h-4 w-48" />
          <div className="ml-auto flex gap-2">
            <div className="aura-skeleton h-8 w-20 rounded-lg" />
            <div className="aura-skeleton h-8 w-8 rounded-full" />
          </div>
        </div>
        <div className="flex min-h-0 flex-1">
          <div className="aura-skeleton hidden h-full w-14 lg:block" />
          <div className="min-w-0 flex-1 space-y-3 p-4">
            <div className="aura-skeleton aspect-video w-full rounded-lg" />
            <div className="aura-skeleton h-40 w-full rounded-xl" />
          </div>
          <div className="aura-skeleton hidden h-full w-[42%] lg:block" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-[100vh] flex-col bg-white">
      {/* Top header bar */}
      <header className="flex h-12 shrink-0 items-center gap-2 border-b border-[var(--aura-border-soft)] px-2 sm:px-3">
        <Link
          href="/meetings"
          className="rounded-md p-1.5 text-aura-gray transition hover:bg-[var(--aura-input-bg)] hover:text-aura-text"
          aria-label="Back to meetings"
        >
          <ArrowLeft className="h-4 w-4" />
        </Link>

        <div className="flex min-w-0 items-center gap-1.5 text-[12px] text-aura-gray-2">
          <Link href="/meetings" className="shrink-0 transition hover:text-[#6C5CE7]">
            #All Meetings
          </Link>
          <span className="shrink-0">/</span>
          <span className="truncate font-medium text-aura-text">{meeting.title}</span>
        </div>
        ...
        <div className="hidden items-center sm:flex">
          ...
          <span className="ml-2 text-[11px] text-aura-gray">
        ...
        <button
          type="button"
          className="rounded-md border border-aura-border p-1.5 text-aura-gray transition hover:bg-[var(--aura-input-bg)] hover:text-aura-text"
          aria-label="Add"
        >
        ...
        <button
          ...
          className="rounded-md p-1.5 text-aura-gray transition hover:bg-red-50 hover:text-red-500"
          aria-label="Delete"
        >
      ...
      <div className="flex min-h-0 flex-1">
        {/* Left: Smart Search */}
        <div className="hidden shrink-0 border-r border-[var(--aura-border-soft)] lg:block">
          <SmartSearchPanel
            meeting={meeting}
        ...
        <div className="min-h-0 min-w-0 flex-1 overflow-y-auto p-4">
          ...
          {soundbites.length > 0 ? (
            <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-[var(--aura-border-soft)] p-2.5">
              <Bookmark className="h-4 w-4 text-[#6C5CE7]" />
              <span className="text-xs font-semibold text-aura-text">Soundbites</span>
              {soundbites.map((s) => (
                <button
                  key={s.id}
                  type="button"
                  onClick={() => onSeek(s.start_time_seconds, s.transcript_line_id)}
                  className="inline-flex items-center gap-1.5 rounded-full bg-aura-soft px-2.5 py-1 text-[11px] font-medium text-[#6C5CE7] transition hover:bg-aura-muted"
                >
                  {s.label}
                  <span className="text-aura-gray-2">
          ...
          <div className="mt-4 rounded-xl border-2 border-dashed border-[#6C5CE7]/50 p-4">
            <h3 className="mb-3 text-sm font-semibold text-aura-text">Action items</h3>
            <ActionItemsPanel
          ...
          <div className="mt-4">
            <button
              ...
              className="text-sm font-semibold text-aura-text transition hover:text-[#6C5CE7]"
            >
              Summary {summaryOpen ? "▾" : "▸"}
            </button>
            {summaryOpen ? (
          ...
          <div className="mt-4 flex gap-2 lg:hidden">
            <button
              ...
              className="aura-btn-secondary flex-1"
            >
        ...
        <aside className="flex w-full min-h-0 min-w-0 flex-col border-l border-[var(--aura-border-soft)] lg:w-[42%]">
          <div className="flex shrink-0 gap-0.5 border-b border-[var(--aura-border-soft)] px-2">
            <button
              type="button"
              onClick={() => setRightTab("transcript")}
              className={`aura-tab ${rightTab === "transcript" ? "aura-tab-active" : ""}`}
            >
              Transcript
            </button>
            <button
              type="button"
              onClick={() => setRightTab("askfred")}
              className={`aura-tab inline-flex items-center gap-1.5 ${
                rightTab === "askfred" ? "aura-tab-active" : ""
              }`}
            >
              <span className="flex h-4 w-4 items-center justify-center rounded bg-aura-soft text-[9px] font-bold text-[#6C5CE7]">
                AI
              </span>
              AskFred
            </button>
      ...
      <div className="flex h-14 shrink-0 items-center justify-center border-t border-[var(--aura-border-soft)] bg-white px-3">
        <div className="flex max-w-3xl items-center gap-2 sm:gap-3">
          <span className="shrink-0 text-[11px] font-medium tabular-nums text-aura-gray">
          ...
          <button
            ...
            className="rounded-lg p-1.5 text-aura-gray transition hover:bg-[var(--aura-input-bg)] hover:text-aura-text"
          ...
          <select
            ...
            className="rounded-md border border-aura-border bg-white px-1.5 py-1 text-[11px] font-semibold text-aura-text outline-none focus:border-[#6C5CE7]"
      ...
      >
        <textarea
          ...
          className="aura-input min-h-[100px]"
