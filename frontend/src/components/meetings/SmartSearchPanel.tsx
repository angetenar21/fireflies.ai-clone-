...
export function SmartSearchPanel({
  ...
  return (
    <div className="flex h-full min-h-0 bg-white dark:bg-aura-bg">
      {/* Icon rail */}
      <div className="flex w-12 shrink-0 flex-col items-center gap-1 border-r border-[#E5E7EB] bg-white py-3 dark:border-aura-border dark:bg-aura-sidebar">
        {rail.map((item) => {
            ...
            <button
              ...
              className={`relative flex h-9 w-9 items-center justify-center rounded-lg transition ${
                isActive
                  ...
                  : "text-[#6B7280] hover:bg-[#F9FAFB] hover:text-aura-text"
              }`}
            >
      ...
      {showSidePanel ? (
        <div className="flex w-[280px] shrink-0 flex-col overflow-hidden border-r border-[#E5E7EB] bg-white dark:border-aura-border dark:bg-aura-bg">
          {smartOpen ? (
            <div className="min-h-0 flex-1 overflow-y-auto p-4">
              <h2 className="mb-4 text-[15px] font-semibold text-aura-text">
                Smart Search
              </h2>

              {/* AI FILTERS */}
              <section className="mb-5">
                <h3 className="mb-2.5 text-[11px] font-semibold uppercase tracking-wide text-aura-gray-2">
                  AI Filters
                </h3>
                <div className="flex flex-wrap gap-2">
                  {filters.map((f) => (
                    <button
                      key={f.label}
                      type="button"
                      className="inline-flex items-center gap-1.5 rounded-full border border-[#E5E7EB] bg-white px-2.5 py-1 text-[12px] font-medium text-aura-text transition hover:border-[#D1D5DB] dark:border-aura-border dark:bg-aura-bg"
                    >
                      <span
                      ...
                      />
                      {f.label}
                      <span className="text-aura-gray-2">{f.count}</span>
                    </button>
                  ))}
                </div>
              </section>
                ...
                <ul className="space-y-2.5">
                  {sentiment.map((s) => (
                    <li key={s.label} className="flex items-center gap-2.5">
                      ...
                      />
                      <span className="flex-1 text-[13px] font-medium text-aura-text">
                        {s.label}
                      </span>
                      <span className="text-[13px] tabular-nums text-aura-gray">
                        {s.pct}%
                      </span>
                    </li>
                  ))}
              ...
              {/* SPEAKER TALKTIME */}
              <section>
                <div className="mb-2.5 grid grid-cols-[1fr_40px_56px] gap-2 text-[10px] font-semibold uppercase tracking-wide text-aura-gray-2">
                  <span>Speakers</span>
                ...
                <ul className="space-y-3">
                  {talkTime.length === 0 ? (
                    <li className="py-4 text-center text-xs text-aura-gray">
                      No speaker data
                    </li>
                  ) : (
                    talkTime.map((s) => (
                      ...
                      >
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          <p className="truncate text-[12px] font-medium text-aura-text">
                            {s.name}
                          </p>
                        </div>
                        <span className="text-center text-[12px] tabular-nums text-aura-gray">
                          {s.wpm}
                        </span>
                        <div className="flex items-center justify-end gap-1.5">
                          <span className="text-[12px] font-semibold tabular-nums text-aura-text">
                            {s.pct}%
                          </span>
                          <div
            ...
            <div className="p-4">
              <h2 className="mb-2 text-[15px] font-semibold text-aura-text">
                Bookmarks
              </h2>
              <p className="text-xs text-aura-gray">
                {soundbiteCount
            ...
            <div className="p-4">
              ...
              <p className="text-xs text-aura-gray">Coming soon.</p>
            </div>
          ) : null}
        </div>
      ) : null}
