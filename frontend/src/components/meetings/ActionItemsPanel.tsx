  ...
  return (
    <div className="space-y-4">
      ...
      <ul className={compact ? "space-y-3" : "space-y-4"}>
        {sorted.length === 0 ? (
          <li className="rounded-lg border border-dashed border-aura-border bg-[var(--aura-input-bg)] px-3 py-6 text-center text-sm text-aura-gray">
            No action items yet.
          </li>
        ) : (
          grouped.map((group) => (
                ...
                {group.items.map((item) => {
                  ...
                  if (editingId === item.id) {
                    return (
                      <li
                        key={item.id}
                        className="rounded-lg border border-[var(--aura-border-soft)] bg-[var(--aura-input-bg)]/60 px-3 py-2.5"
                      >
                        <div className="space-y-2">
                          ...
                          <div className="flex justify-end gap-2">
                            <button
                              ...
                              className="aura-btn-secondary h-8 px-2.5 text-xs"
                            >
                            ...
                            <button
                              ...
                              disabled={busyId === item.id}
                              className="aura-btn-primary h-8 px-2.5 text-xs"
                            >
                              {busyId === item.id ? (
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        className="mt-1 h-3.5 w-3.5 shrink-0 cursor-pointer rounded border-aura-border text-[#6C5CE7] opacity-60 transition focus:ring-[#6C5CE7]/30 group-hover:opacity-100"
                      />
                      <div className="min-w-0 flex-1">
                        <p
                          className={`text-sm leading-snug ${
                            item.is_completed
                              ? "text-aura-gray-2 line-through"
                              : "text-aura-text"
                          }`}
                        >
                          <span className="mr-1 text-aura-gray-2">•</span>
                          {item.text}{" "}
                          {onTimestampClick ? (
                            <button
                              type="button"
                      ...
                      <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition group-hover:opacity-100">
                        <button
                          ...
                          disabled={busyId === item.id}
                          className="rounded-md p-1 text-aura-gray-2 transition hover:bg-[#F3F0FF] hover:text-[#6C5CE7]"
                        ...
                        <button
                          ...
                          disabled={busyId === item.id}
                          className="rounded-md p-1 text-aura-gray-2 transition hover:bg-red-50 hover:text-red-500 disabled:opacity-40"
      ...
      <form
        onSubmit={onSubmit}
        className="space-y-2 border-t border-[var(--aura-border-soft)] pt-4"
      >
        <p className="text-xs font-semibold uppercase tracking-wide text-aura-gray-2">
          Add action item
        </p>
        <input
          type="text"
