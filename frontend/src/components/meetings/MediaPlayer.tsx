...
export const MediaPlayer = forwardRef<MediaPlayerHandle, MediaPlayerProps>(
  function MediaPlayer(
    ...
    return (
      <div className={chrome === "video" ? "" : "aura-card p-4"}>
        <audio
          ref={audioRef}
          src={src}
          preload="metadata"
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <span className="text-[11px] font-medium tabular-nums text-aura-gray">
            ...
            <div className="relative">
              <button
                ...
                className="rounded-md border border-aura-border px-2 py-1 text-[11px] font-semibold text-aura-text transition hover:bg-[var(--aura-input-bg)]"
              >
                {speed}x
              </button>
              {speedOpen ? (
                <div className="absolute bottom-full left-0 z-20 mb-1 min-w-[72px] overflow-hidden rounded-lg border border-aura-border bg-aura-bg py-1 shadow-lg">
                  {SPEEDS.map((s) => (
                    <button
                      key={s}
                      type="button"
                      ...
                      className={`block w-full px-3 py-1.5 text-left text-[11px] font-medium transition hover:bg-aura-soft ${
                        speed === s ? "text-aura-purple" : "text-aura-text"
                      }`}
                    >
                      {s}x
                    </button>
            ...
            {usingDemoTimer ? (
              <span className="text-[10px] text-aura-gray-2">demo timer</span>
            ) : null}
          </div>
          ) : null}
        </div>
