    ...
    <div
      className={
        embedded
          ...
          : "aura-card flex min-h-0 flex-1 flex-col"
      }
    >
      <div className="flex flex-col gap-3 border-b border-[var(--aura-border-soft)] p-4 sm:flex-row sm:items-center">
        {embedded ? null : (
          <h2 className="text-sm font-semibold text-aura-text">Transcript</h2>
        )}
        <div className="flex flex-1 items-center gap-2">
          <label className="relative min-w-0 flex-1">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-aura-gray-2" />
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Find or Replace"
              className="w-full rounded-lg border border-aura-border bg-[var(--aura-input-bg)] py-2 pl-9 pr-3 text-sm text-aura-text outline-none focus:border-aura-purple focus:ring-2 focus:ring-aura-purple/15"
            />
          </label>
          {debounced ? (
            <div className="flex items-center gap-1">
              <span className="whitespace-nowrap text-xs text-aura-gray">
                {matches.length
              ...
              <button
                ...
                className="rounded-md border border-aura-border p-1.5 text-aura-gray hover:bg-[var(--aura-row-hover)] disabled:opacity-40"
      ...
      >
        {lines.length === 0 ? (
          <p className="px-3 py-10 text-center text-sm text-aura-gray">
            ...
            return (
              <div
                ...
                className={`group flex w-full gap-2 rounded-xl px-2 py-2.5 transition sm:gap-3 sm:px-3 ${
                  active
                    ? "bg-[var(--aura-highlight)] ring-1 ring-aura-purple/20"
                    : isSavedHighlight
                      ? "bg-[var(--aura-amber)]"
                      : isMatchLine
                        ? "bg-[var(--aura-amber)]/80"
                        : "hover:bg-[var(--aura-row-hover)]"
                }`}
              >
                <button
                  type="button"
